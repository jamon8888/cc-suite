...
1.  **Load Context Profiles**:
    - Read `${CLAUDE_PLUGIN_ROOT}/data/voice-dna.json` to match the user's voice precisely.
    - Read `${CLAUDE_PLUGIN_ROOT}/data/icp.json` to align with the audience's pain points.
    - Read `${CLAUDE_PLUGIN_ROOT}/data/business-profile.json` to ensure alignment with offers.
2.  **Check Source Material**:
    - If repurposing, check wiki articles via `wiki/` for relevant sources.
...
- Never link to internal wiki or draft resources in the article output — those are drafts, not live pages
...
- **Internal Links**: Suggest linking to relevant published content.
