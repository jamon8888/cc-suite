---
description: "Apprendre: ingerer une source dans ta base de connaissances wiki. URLs, fichiers, texte."
argument-hint: "<url|fichier|\"texte\"> [--type articles|papers|repos|notes|data] [--title \"Titre\"] [--wiki <name>] [--new-topic <name>]"
...
Check if `${CLAUDE_PLUGIN_ROOT}/data/voice-dna.json` exists and is populated. If empty, suggest: 'Lance /copywriter:start d'abord pour configurer ton profil.'
...
Read the ingestion protocol at `${CLAUDE_PLUGIN_ROOT}/skills/wiki-core/references/ingestion.md` and the structure spec at `${CLAUDE_PLUGIN_ROOT}/skills/wiki-core/references/wiki-structure.md`. Then ingest source material.
...
8. Check uncompiled source count. If 5+, nudge: "Tu as N sources non compilees. Les compiler en articles ? (o/n)"
