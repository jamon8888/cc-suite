---
description: "Recherche approfondie multi-agents. Persiste dans ta base wiki. Mode these pour valider des angles."
...
#### Phase 1b: Load ICP Context

Read `${CLAUDE_PLUGIN_ROOT}/data/icp.json` to get `icp.pain_points` and `icp.vocabulary`. Include these in each agent's prompt.
...
**Audience**: Search for content relevant to this audience: {icp.pain_points}. Prioritize practitioner sources over academic.
...
4. For each compiled article, generate 2-4 `content_angles` in frontmatter -- suggested hooks for copywriting.
...
Prochaine etape: /copywriter:ecrire blog ou /copywriter:calendrier
