...
> "No active applications to follow up on. Apply to some roles first with `/job-os` and come back when they're aging."
...
Reuse the message framework: 3 sentences, 300 character max.
- Hook specific to company → proof point → soft ask
- Suggest the user run `/job-os message {company}` to find the right person first
...
> - Trying a different contact via `/job-os message`
...
**To:** {email or "No contact found — run `/job-os message` first"}
