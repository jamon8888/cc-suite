...
1.  **Load Context Profiles**:
    ...
    - Read `${CLAUDE_PLUGIN_ROOT}/data/icp.json` to align with the audience's pain points and jargon.
    - Read `${CLAUDE_PLUGIN_ROOT}/data/business-profile.json` to ensure the post aligns with overall positioning.
2.  **Identify Post Objective**: Is the goal _Brand Awareness_, _Lead Generation_, or _Topical Authority_?
3.  **Check Source Material**: If the user wants to repurpose a blog post or transcript, check wiki articles for relevant sources.
