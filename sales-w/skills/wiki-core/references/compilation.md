...
9. During compilation, if the source relates to an identifiable company/market, set `entity_type` (account|prospect|competitor|market) and appropriate `freshness_threshold` (competitor=30, prospect=45, account=60, market=90). Set `last_verified` to today's date. Also set `sales_angles` in frontmatter.
10. Generate 2-4 `sales_angles` per article: objection opportunities, pain point alignments, competitive advantages.
