---
description: "Initialiser le plugin Sales: profil vendeur, ICP, Voice DNA et base sales-wiki."
argument-hint: "[--update voice|icp|sales-profile]"
...
```
/sales:demarrer
/sales:demarrer --update icp        # Update specific config
/sales:demarrer --scan deck.pdf     # Auto-configure from file
```
...
All data is stored locally in `${CLAUDE_PLUGIN_ROOT}/data/sales-profile.json`.
...
2. Find prospects: /sales:prospecter
3. Draft outreach: /sales:creer
```
...
```python
config_file = "${CLAUDE_PLUGIN_ROOT}/data/sales-profile.json"
...
language = ask_choice(["English", "Francais", "Bilingual"])
lang_code = "fr" if language == "Francais" else "en"
...
mkdir -p data/{0-Inbox,1-Projets,3-Ressources,4-Archives}
mkdir -p data/LinkedIn
mkdir -p data/1-Projets/{active-deals,campaigns}
...
if not file_exists("data/LinkedIn/prospects.md"):
    write_file("data/LinkedIn/prospects.md", PROSPECT_TEMPLATE)

if not file_exists("data/LinkedIn/activity_log.md"):
    write_file("data/LinkedIn/activity_log.md", LOG_TEMPLATE)
...
1. Read `${CLAUDE_PLUGIN_ROOT}/data/sales-profile.json` to get the business niche/territory
2. Derive a wiki topic slug (lowercase, hyphens, max 40 chars)
3. Resolve HUB: try `$HOME/sales-wiki/_index.md`, else `~/.config/llm-sales-wiki/config.json`, else `$HOME/sales-wiki`
4. If HUB doesn't exist, create it: `wikis.json` + `_index.md` + `log.md` + `topics/`
5. Create `HUB/topics/<slug>/` following wiki init protocol from `${CLAUDE_PLUGIN_ROOT}/skills/wiki-core/references/wiki-structure.md`
6. Register in `HUB/wikis.json` and update hub `_index.md`

### Phase 3: Bootstrap Research

Ask: "Veux-tu que je recherche tes principaux concurrents ? (5 min)"
- If yes: invoke `Skill: sales:recherche` with top 3 competitors and `--sources 10`
- If no: suggest running `/sales:recherche` later

### Profile Versioning

Before overwriting `sales-profile.json`, copy to `data/sales-profile.previous.json`.

### --update flag

- `--update voice`: re-run only Voice DNA interview
- `--update icp`: re-run only ICP interview
- `--update sales-profile`: re-run only sales profile interview
