...
After delivering the synthesis, automatically bridge findings to design work:

### Flow Recommendations
For each high-priority recommendation, identify whether it implies a **flow problem** (user gets stuck, wrong path, missing step) or a **screen problem** (confusing interface on a specific screen).

- **Flow problem** → Offer: "This finding suggests the [X] journey needs remapping. Run `/flow [user goal]` to diagram the corrected path."
- **Screen problem** → Offer: "Participants struggled with [screen name]. Run `/wireframe [screen]` to redesign it against these findings."

### Design Implications Table
Append to the synthesis output:
...
|---------|------|-----------------|
| [Finding] | Flow problem | `/flow [user goal]` |
| [Finding] | Screen problem | `/wireframe [screen name]` |
| [Finding] | Copy problem | `/ux-copy [context]` |
| [Finding] | System problem | `/design-system extend [pattern]` |
```

This table turns research insights directly into actionable design commands — closing the gap between "what we learned" and "what we build next."
