...
name: wiki-core
description: >
  Copywriter plugin's knowledge base core. Activates when user works with wiki
  directories, mentions knowledge base management, or uses /wiki commands.
  Also activates when user says "wiki", "knowledge base", or when ~/wiki/
...
The hub defaults to `~/wiki/`. If `~/wiki/_index.md` exists, HUB = `$HOME/wiki` — done. Otherwise read `~/.config/llm-wiki/config.json` and use `resolved_path`. If no config exists, HUB = `$HOME/wiki` (for initialization). See `references/hub-resolution.md` for edge cases (iCloud, spaces, symlinks).
